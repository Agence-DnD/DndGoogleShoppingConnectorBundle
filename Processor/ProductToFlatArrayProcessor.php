 ...
 *
 * @author    Florian Fauvel <willy.mesnage@akeneo.com>
...
class ProductToFlatArrayProcessor extends AbstractConfigurableStepElement implements ItemProcessorInterface
{
    ...
    public function process($product)
    {
        ...
        $this->productBuilder->addMissingProductValues(
            $product,
            [$contextChannel],
            [$this->localeRepository->findOneByIdentifier($this->getLocale())]
        );
        ...
        $category = preg_match('/(?<=google_category_).*?(?=\,)/', $data['product']['categories'], $googleCategory);
        if (1 == $category) {
            $data['product']['categories'] = $googleCategory[0];
